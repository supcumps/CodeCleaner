...
Protected Class CodeParser
		...
		Sub ParseFileContent(content As String, fileName As String)
		  ...
		  For Each line As String In lines
		    ...
		    ElseIf IsEndMethodStatement(trimmedLine) Then
		      // Fallback: bare End Sub / End Function (belt-and-suspenders)
		      FinalizeMethod(context)
	...
	#tag Method, Flags = &h0
		Function IsEndMethodStatement(line As String) As Boolean
		  // Returns True ONLY for the closing keyword of a Sub/Function/Property accessor.
		  // Does NOT match End If / End For / End While / End Select / End Try etc.
		  // #tag EndMethod / #tag EndEvent are handled separately in ParseFileContent.
		  Var upper As String = line.Uppercase.Trim
		  Return upper = "END SUB" Or _
	...
	#tag Method, Flags = &h0
		Sub ScanProjectForDeclarations(projectFile As FolderItem)
		  ...
		  // it won't pick up orphaned files, and it will flag manifest entries
		  // whose backing file is missing on disk.
		  
		  If projectFile = Nil Or Not projectFile.Exists Then
		    Logger.Log("ScanProjectForDeclarations: project file is missing")
		    Return
		  End If
		  
		  Var entries() As ManifestEntry = ParseProjectManifest(projectFile)
		  
		  For Each entry As ManifestEntry In entries
		    If entry.IsResolved Then
		      ProcessSourceFile(entry.SourceFile)
		    Else
		      Logger.Log("ScanProjectForDeclarations: skipping unresolved entry " + entry.Name + " (" + entry.FileName + ")")
		    End If
		  Next
	...
	#tag Method, Flags = &h0
		Function ParseProjectManifest(projectFile As FolderItem) As ManifestEntry()
		  ...
		  // Each entry's SourceFile is the resolved FolderItem (Nil if missing on disk).
		  
		  Var entries() As ManifestEntry
		  
		  If projectFile = Nil Or Not projectFile.Exists Then
		    Logger.Log("ParseProjectManifest: project file is missing")
		    Return entries
		  End If
		  
		  Var parentFolder As FolderItem = projectFile.Parent
		  If parentFolder = Nil Then
		    Logger.Log("ParseProjectManifest: could not resolve parent folder of " + projectFile.Name)
		    Return entries
		  End If
		  
		  Var tis As TextInputStream = TextInputStream.Open(projectFile)
		  If tis = Nil Then
		    Logger.Log("ParseProjectManifest: could not open " + projectFile.Name)
		    Return entries
		  End If
		  
		  Var fileContent As String = tis.ReadAll
		  tis.Close
		  
		  Var lines() As String = fileContent.Split(EndOfLine)
		  
		  For Each rawLine As String In lines
		    Var line As String = rawLine.Trim
		    If line.Trim = "" Then Continue
		    
		    Var elementType As String = ""
		    
		    If line.BeginsWith("Class=") Then
		      elementType = "CLASS"
		    ElseIf line.BeginsWith("Module=") Then
		      elementType = "MODULE"
		    ElseIf line.BeginsWith("DesktopWindow=") Then
		      elementType = "WINDOW"
		    ...
		    Var entry As ManifestEntry = ParseManifestLine(line, elementType, parentFolder)
		    If entry <> Nil Then
		      entries.Add(entry)
		    End If
		  Next
	...
	#tag Method, Flags = &h21
		Function ParseManifestLine(line As String, elementType As String, parentFolder As FolderItem) As ManifestEntry
		  ...
		  If equalsPos < 0 Then Return Nil
		  
		  Var afterEquals As String = line.Mid(equalsPos + 2)
		  Var parts() As String = afterEquals.Split(";")
		  
		  If parts.Count < 2 Then Return Nil
		  
		  Var entryName As String = parts(0).Trim
		  Var fileName As String = parts(1).Trim
		  
		  If entryName.Trim = "" Or fileName.Trim = "" Then Return Nil
		  
		  Var resolvedFile As FolderItem = parentFolder.Child(fileName)
		  If resolvedFile = Nil Or Not resolvedFile.Exists Then
		    Logger.Log("ParseManifestLine: manifest references missing file " + fileName + " for " + entryName)
		    resolvedFile = Nil
		  End If
