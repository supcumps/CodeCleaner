...
Protected Class ProjectAnalyzer
	...
	#tag Method, Flags = &h0
		Sub AnalyzeFileForRelationships(content As String)
		  ...
		  For Each line As String In lines
		    ...
		    cp.ProcessLineForRelationships(line, currentModule, currentClass, _
	...
	#tag Method, Flags = &h0
		Sub AnalyzeHighComplexity(method As CodeElement)
	...
	#tag Method, Flags = &h0
		Sub AnalyzeLongMethod(method As CodeElement)
	...
	#tag Method, Flags = &h0
		Sub AnalyzeManyParameters(method As CodeElement)
	...
	#tag Method, Flags = &h0
		Sub AnalyzeMissingErrorHandling(method As CodeElement)
	...
	#tag Method, Flags = &h21
		Private Function RegExEscape(s As String) As String
		  // Escape regex metacharacters so an identifier name can be safely embedded
		  // in a SearchPattern. Xojo identifiers are normally just letters, digits, and
		  // underscores, but this guards against any stray characters reaching here.
	...
	#tag Method, Flags = &h21
		Private Function CleanCodeForAnalysis(content As String) As String
		  ...
		  // Remove // line comments (the dominant Xojo comment style, including the
	...
	#tag Method, Flags = &h21
		Private Sub DetectMethodCalls(code As String, callingMethod As CodeElement)
		  ...
		  If code.Trim = "" Then
		    Logger.Log("  Code is EMPTY!")
		    Return
		  End If
		  
		  // Strip comments and string literals first. CodeParser stores a leading
		  // "// Public Sub ClassName.MethodName(...)" header comment in every method's
		  // Code property, and without stripping it that header text matches this very
		  // method's own name (e.g. "Log(" inside "// Public Sub Logger.Log(message...)"),
		  // producing false self-referential call results.
		  Var cleanCode As String = CleanCodeForAnalysis(code)
		  
		  // Show first 200 chars of code
		  Var preview As String = cleanCode.Left(Min(200, cleanCode.Length))
		  ...
		  // Check all known methods to see if they're genuinely called in this code
		  For Each methodElement As CodeElement In methods
		    Var methodName As String = methodElement.Name
		    If methodName.Trim = "" Then Continue
		    
		    // A method heuristically "calling itself" via text search is almost always
		    // a false positive (its own name appearing in its own declaration, a log
		    // message, or a recursive-looking but unrelated comment) rather than real
		    // recursion, so we don't attempt to detect self-calls here at all.
		    If methodElement.FullPath = callingMethod.FullPath Then Continue
		    
		    Var foundCall As Boolean = False
		    
		    // Whole-identifier match: MethodName must be a complete word, optionally
		    // preceded by "." (for Class.MethodName / instance.MethodName calls) and
		    // optionally followed by "(" (calls) or AddressOf usage. \b prevents a
		    // short name like "id" or "Log" from matching inside longer identifiers.
		    rx.SearchPattern = "(?:\.|\b)" + RegExEscape(methodName) + "\s*\("
		    If rx.Search(cleanCode) <> Nil Then
		      foundCall = True
		    End If
		    
		    If Not foundCall Then
		      rx.SearchPattern = "AddressOf\s+" + RegExEscape(methodName) + "\b"
		      If rx.Search(cleanCode) <> Nil Then
		        foundCall = True
		      End If
		    End If
  ...
  // Framework event/lifecycle methods invoked directly by the Xojo runtime.
  // Constructor/Destructor are handled separately (need "New ClassName" check).
  Var systemMethods() As String = Array("Open", "Close", _
  ...
  "Action", "Pressed", "ValueChanged", "TextChanged", "SelectionChanged", "DropObject", _
  "DragEnter", "DragExit", "DragOver", "DragWithin", _
  "UnhandledException", "NewDocument", "OpenDocument", "HandleURL")
  ...
  Logger.Log("Total elements to check: " + allElements.Count.ToString)
  Logger.Log("Cleaned text length: " + cleanedText.Length.ToString)
  ...
  For Each element As CodeElement In allElements
    ...
    Var escapedName As String = RegExEscape(name)
    
    Select Case element.ElementType
    Case "METHOD"
      If IsSystemMethod(name) Then
        // Framework event/lifecycle handlers (Pressed, ValueChanged, Opening, etc.)
        // are invoked directly by the Xojo runtime, never by name in user source.
        // Always treat as used.
        element.IsUsed = True
        Logger.Log("Event handler (runtime-invoked, always used): " + element.FullPath)
        
      ElseIf name = "Constructor" Or name = "Destructor" Then
        // Constructors/Destructors are called via "New ClassName(...)" not by the
        // literal text "Constructor(". Mark as used if the owning class is instantiated.
        // FullPath is "Class.Constructor" or "Module.Class.Constructor" -- take the
        // second-to-last segment to get the owning class name.
        Var ownerClass As String = element.ParentClass
        If ownerClass.Trim = "" And element.FullPath.Contains(".") Then
          Var ownerParts() As String = element.FullPath.Split(".")
          Var lastIdx As Integer = ownerParts.Count - 1
          If lastIdx >= 1 Then
            ownerClass = ownerParts(lastIdx - 1)
          End If
        End If
        
        Var isCalled As Boolean = False
        If ownerClass.Trim <> "" Then
          rx.SearchPattern = "\bNew\s+" + RegExEscape(ownerClass) + "\b"
          isCalled = (rx.Search(cleanedText) <> Nil)
          If Not isCalled Then
            // Also check "As OwnerClass" for typed variables / parameters
            rx.SearchPattern = "\bAs\s+" + RegExEscape(ownerClass) + "\b"
            isCalled = (rx.Search(cleanedText) <> Nil)
          End If
        End If
        element.IsUsed = isCalled
        Logger.Log("Constructor " + element.FullPath + " -> owner=" + ownerClass + " used=" + isCalled.ToString)
        
      Else
        // Regular methods: whole-identifier match, optionally qualified
        // ("." + name) and followed by "(" for a real call
        rx.SearchPattern = "(?:\.|\b)" + escapedName + "\s*\("
        Var isCalled As Boolean = (rx.Search(cleanedText) <> Nil)
        
        If Not isCalled Then
          rx.SearchPattern = "AddressOf\s+" + escapedName + "\b"
          isCalled = (rx.Search(cleanedText) <> Nil)
        End If
    ...
    Case "CLASS"
      // App is always instantiated implicitly by the Xojo runtime.
      If name = "App" Then
        element.IsUsed = True
      Else
        rx.SearchPattern = "\bNew\s+" + escapedName + "\b"
        Var isCalled As Boolean = (rx.Search(cleanedText) <> Nil)
        
        If Not isCalled Then
          rx.SearchPattern = "\bAs\s+" + escapedName + "\b"
          isCalled = (rx.Search(cleanedText) <> Nil)
        End If
        
        If Not isCalled Then
          rx.SearchPattern = "\bInherits\s+" + escapedName + "\b"
          isCalled = (rx.Search(cleanedText) <> Nil)
        End If
        
        // A class containing framework event handlers is runtime-instantiated
        // even if no "New ClassName" appears in source (e.g. Window subclasses).
        If Not isCalled Then
          For Each candidate As CodeElement In allElements
            If candidate.ElementType = "METHOD" And IsSystemMethod(candidate.Name) And _
              candidate.FullPath.BeginsWith(name + ".") Then
              isCalled = True
              Exit For
            End If
          Next
    ...
    Case "MODULE"
      rx.SearchPattern = "\b" + escapedName + "\."
      element.IsUsed = (rx.Search(cleanedText) <> Nil)
    ...
    Case "PROPERTY", "VARIABLE"
      ...
      If Not isCalled Then
        rx.SearchPattern = "\b" + escapedName + "\b"
        isCalled = (rx.Search(cleanedText) <> Nil)
      End If
  ...
  Logger.Log("=== MARKUSEDELEMENTS COMPLETED ===")
	...
	#tag Method, Flags = &h0
		Sub ScanProject(projectFile As FolderItem)
		  ...
		  Var cp As New CodeParser
		  cp.ScanProjectForDeclarations(projectFile)
		  
		  Var parsed() As CodeElement = cp.GetAllElements()
		  For Each element As CodeElement In parsed
		    AllElements.Add(element)
		    ElementLookup.Value(element.FullPath) = element
		    Select Case element.ElementType
	...
	#tag Method, Flags = &h21
		Sub ScanFileForRelationships(item As FolderItem)
		  ...
		  Var cp As New CodeParser
  If Not cp.IsXojoSourceFile(item) Then
		    Return
		  End If
